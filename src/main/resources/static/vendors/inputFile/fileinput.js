...
//                '{uploadIcon}</button>';
            ...
            self.defaults = {
                ...
                fileActionSettings: {
                    ...
                    indicatorError: '<i class="glyphicon glyphicon-exclamation-sign text-danger"></i>',
                    indicatorLoading: '<i class="glyphicon glyphicon-hand-up text-muted"></i>',
                    indicatorNewTitle: 'Ainda nao foi carregado',
        ...
        _readFiles: function (files) {
        	...
        	if(files){
	        	...
	        	$.each(files, function (key, data) {
	        		...
	        		var tamanho = (data.size || 0) / 1000;
			        	if (tamanho > 25600) {
			        		$('#alerts_erro_documento_origem').append(
			        			$('<div>')
			        			.attr('class','alert alert-danger alert-dismissible ' + ' alert_valida_arquivo'+data.size)
			        			.append(
			        					$('<span>').text('O Arquivo '+ files[key].name +' excede o tamanho maximo de upload '+ files[key].size+'  permitido de 25600 (25MB), remova-o para enviar os arquivos!')
			        			).append(
			        					$('<button>').attr('type','button')
			        					.attr('class','close')
			        					.attr('aria-label','Close')
			        					.attr('onclick',"$('.alert_valida_arquivo"+data.size+"').hide()")
			        					.append(
			        	...
			        	excluir.push(files[key]);
//			            $('#div_alert_valida_arquivo').removeAttr('hidden');
//			            nome = nome +' ' +files[0].name;
//			           
			    	  }
	        	});
...
//	        
	        	...
	        	//se existe algum arquivo maior que 25 mb
	        	if(excluir!= undefined && excluir.length > 0){
	        		files = [];		
	        		//limpa o input
	        		self._resetUpload();
	                self.clearStack();
	                ...
	                self._resetPreview();
	                self.reset();
	                 self.$container.find('.fileinput-remove').addClass('hide');
	        	}
	        }
...
//        	console.log()
//        	if (files[0].size > 25600) {
//                $('#div_alert_valida_arquivo').removeAttr('hidden');
//        	   document.getElementById('span_valida_arquivo').innerHTML ='Arquivo'+ nome +' excede o tamanho maximo de upload permitido de ( 25MB )';
//       		//              self.isError = throwError(msg, file, previewId, i);
//       		              return;
//        	}
//        	if(files[0].size > 25600){
...
//        	}
            ...
            readFile = function (i) {
                ...
                if (caption.length === 0) {
                    ...
                    self.isError = throwError(msg, file, previewId, i);
//                    if(!$('#salvar_dado_origem_bo').hasClass('disabled')){
                    	$('#salvar_dado_origem_bo').addClass('disabled');
//                    }
                    return;
                }
...
//                    }
                ...
                if (!$h.isEmpty(fileTypes) && $h.isArray(fileTypes)) {
                    ...
                    if (fileCount === 0) {
                        ...
                        self.isError = throwError(msg, file, previewId, i);
//                        if(!$('#salvar_dado_origem_bo').hasClass('disabled')){
//                        	$('#salvar_dado_origem_bo').addClass('hidden');
//                        }
                        return;
                    }
                }
...
//            console.log(self.$element.attr('name'));
            if (self.isClickable) {
    ...
    $.fn.fileinput = function (option) {
        ...
        this.each(function () {
            var self = $(this), data = self.data('fileinput'), options = typeof option === 'object' && option,
                ...
                lang = options.language || self.data('language') || $.fn.fileinput.defaults.language || 'pt', opt;
            if (!data) {
                if (theme) {
                    t = $.fn.fileinputThemes[theme] || {};
                }
                if (lang !== 'pt' && !$h.isEmpty($.fn.fileinputLocales[lang])) {
                    l = $.fn.fileinputLocales[lang] || {};
                }
                opt = $.extend(true, {}, $.fn.fileinput.defaults, t, $.fn.fileinputLocales.pt, l, options, self.data());
...
//    };
    ...
    $.fn.fileinputLocales.pt = {
            fileSingle: 'arquivo',
            filePlural: 'arquivos',
            browseLabel: 'Pesquisar &hellip;',
            removeLabel: 'Remover',
            removeTitle: 'Limpar arquivos Selecionados',
            cancelLabel: 'Cancelar',
            cancelTitle: 'Anular o upload em andamento',
//            uploadLabel: 'Upload',
//            uploadTitle: 'Upload selected files',
            msgNo: 'Nao',
            msgNoFilesSelected: 'Anular o upload em andamento',
            msgCancelled: 'Cancelado',
            msgZoomModalHeading: 'Visualizacao detalhada',
            msgSizeTooSmall: 'Arquivo "{name}" (<b>{size} KB</b>) e muito pequeno e deve ser maior que <b>{minSize} KB</b>.',
            msgSizeTooLarge: 'Arquivo "{name}" (<b>{size} KB</b>) excede o tamanho maximo de upload permitido <b>{maxSize} KB</b> (25 MB).',
            msgFilesTooLess: 'Voce deve selecionar pelo menos <b>{n}</b> {files} para fazer upload.',
            msgFilesTooMany: 'Numero de arquivos selecionados para upload <b>({n})</b> excede o limite maximo permitido de <b>{m}</b>.',
            msgFileNotFound: 'Arquivo "{name}" nao encontrado!',
            msgFileSecured: 'excede as restricoes de segurança que impedem a leitura do limite minimo permitido "{name}".',
            msgFileNotReadable: 'Arquivo "{name}" nao e legivel.',
            msgFilePreviewAborted: 'Visualizacao de arquivo anulada por "{name}".',
            msgFilePreviewError: 'Ocorreu um erro durante a leitura do arquivo "{name}".',
            msgInvalidFileName: 'Caracteres invalidos ou sem suporte no nome do arquivo "{name}".',
            msgInvalidFileType: 'Tipo invalido para o arquivo "{name}". Somente "{types}" arquivos sao suportados.',
            msgInvalidFileExtension: 'Extensao invalida para o arquivo "{name}". Somente "{extensions}" arquivos sao suportados.',
            msgFileTypes: {
                'image': 'image',
                'html': 'HTML',
                'text': 'text',
            ...
            msgUploadAborted: 'O upload do arquivo foi cancelado',
            msgUploadThreshold: 'Processando...',
            msgUploadBegin: 'Inicializando...',
            msgUploadEnd: 'Feito',
            msgUploadEmpty: 'Nenhum dado valido disponivel para upload.',
            msgValidationError: 'Erro de validacao',
            msgLoading: 'Carregando arquivo {index} de {files} &hellip;',
            msgProgress: 'Carregando arquivo {index} of {files} - {name} - {percent}% completo.',
            msgSelected: '{n} {files} selecionado(s)',
            msgFoldersNotAllowed: 'Apenas arraste e solte! {n} as pastas soltas foram ignoradas.',
            msgImageWidthSmall: 'Largura do arquivo de imagem "{name}" deve ser pelo menos {size} px.',
            msgImageHeightSmall: 'Altura do arquivo de imagem "{name}" deve ser pelo menos {size} px.',
            msgImageWidthLarge: 'Largura do arquivo de imagem "{name}" nao pode exceder {size} px.',
            msgImageHeightLarge: 'Altura do arquivo de imagem "{name}" nao pode exceder {size} px.',
            msgImageResizeError: 'Nao foi possivel redimensionar as dimensoes da imagem.',
            msgImageResizeException: 'Error while resizing the image.<pre>{errors}</pre>',
            msgAjaxError: 'Algo deu errado com a {operation} operacao. Por favor tente novamente mais tarde!',
            msgAjaxProgressError: 'Falha ao realizar a operacao {operation}',
            ajaxOperations: {
                deleteThumb: 'apagar arquivo',
                uploadThumb: 'upload de arquivo',
                uploadBatch: 'upload de arquivo em lote',
                uploadExtra: 'upload de dados'
            },
            dropZoneTitle: 'Pesquise por arquivos &hellip;',
            dropZoneClickTitle: '<br>(ou click para selecionar {files})',
            previewZoomButtonTitles: {
                prev: 'Ver arquivo anterior',
                next: 'Ver proximo arquivo',
                toggleheader: 'Alternar Cabecalho',
                fullscreen: 'Alternar para o modo tela cheia',
                borderless: 'Alternar modo sem borda',
                close: 'Fechar pre-visualizacao detalhada'
            }
        };
     ...
     */
    $(document).ready(function () {
        var $input = $('input.file[type=file]').attr('name', 'files');
        if ($input.length) {
        	console.log($input);
            $input.fileinput();
            //updateSize();
...
function updateSize() {
    var nBytes = 0,
            oFiles = document.getElementById("file-upload-demo").files,
            nFiles = oFiles.length;
    
//    addMultPart($('#fileInput').val());
